...
const ItemPost: React.FC<Props> = ({ data, modal = false }) => {
    ...
    useEffect(() => {
        ...
        if (newData.MediaItems.length > 0) {
            ...
            const imgs: {}[] = newData.MediaItems.map((item) => ({
                id: item.id,
                mediaType: item.mediaType,
                mediaUrl: url + item.mediaUrl,
            }));
            setViews(items);
            setImageViewer(imgs);
        }
    ...
    const handleConfirmDelete = () => {
        dispatch(deletePost({ id: newData.id }));
    };
    ...
    const hasUserReacted = (postId: number, userId: number) => {
        return postSelector.posts.some(
            (post: any) =>
    ...
    const moreMenu = [
        {
            icon: <EditIcon className={cx('icon-more')} />,
            title: t('comment.btn_edit'),
            action: onEdit,
            rule: checkUser,
        },
        {
    ...
    return (
        <div className={cx('wrapper')}>
            ...
            <div className={cx('react')}>
                {!checkUser && (
                    <div className={cx('btn-follow', 'btn-foot-post')}>
                        {!tick ? (
                ...
                <div className={cx('note-liked')}>
                    <span className={cx('text-liked')}>
                        {data.Reacts.length > 0 && data.Reacts.length} {data.Reacts.length > 0 && t('noti.liked')}{' '}
                    </span>
                </div>
            </div>
                ...
                <div className={cx('list-comment')}>
                    ...
                    {newData.children.length > 0 && !modal && !isModalOpen && (
                        <button onClick={openModal} className={cx('show-more')}>
                            ...
                            {newData.children.length > MAX_INITIAL_COMMENTS &&
                                newData.children.length - MAX_INITIAL_COMMENTS}
                        </button>
                    )}
                </div>
            ...
            {editOpen && (
                <div className={cx('edit-viewer')}>
                    <div className={cx('box-edit')}>
                        ...
                        <FormPost
                            className={cx('custom-form-edit')}
                            initialize={{ body: newData.body, mediaItems: imgViewer, id: newData.id }}
                        />
                    </div>
                </div>
            )}
